...
const temp = ref([
    {
        ...
        checked: true,
        courseId: 1,
        text: '在 vite 中，主要通过 ModuleGraph 和 ModuleNode 来建立各模块依赖关系，ModuleGraph 记录模块及模块的所有依赖，ModuleNode 记录模块节点具体信息模块依赖图在项目启动时通过 ModuleGraph 类创建一个实例ts复制代码const moduleGraph: ModuleGraph = new ModuleGraph((url, ssr) =>container.resolveId(url, undefined, { ssr }),)ModuleGraph 主要通过三个 Map 和一个 Set 来记录模块信息，包括urlToModuleMap：原始请求 url 到模块节点的映射，如 /src/index.tsx（vite 中的每个模块 url 是唯一的）idToModuleMap：模块 id 到模块节点的映射，id 是原始请求 url 经过 resolveId 钩子解析后的结果fileToModulesMap：文件到模块节点的映射，由于单文件可能包含多个模块，如 .vue 文件，因此 Map 的 value 值为一个集合safeModulesPath：录被认为是“安全”的模块路径，安全路径不需要模块转换和处理',
        score: 10,
    },
    {
        title: '562656565',
            ...
            .then((res: any) => {
                if (res.code == 200) {
                    ...
                    EvaluationList.value.forEach((item: any, index: number) => {
                        if (!item.checked) {
                            EvaluationList.value[index] = true
                        }
                    })
                ...
                <van-pull-refresh v-model="reLoad" @refresh="refreshList">
                    ...
                    <div v-else class="list">
                        <van-list v-model:loading="loading" :finished="finished" finished-text="没有更多了"
                            ...
                            <van-cell v-for="(item, index) in temp" :key="index" @click="openDetail(item.courseId)"
