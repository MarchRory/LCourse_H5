...
onActivated(() => {
  if (scrollTop.value) {
    document.querySelector(".van-list").scrollTop = scrollTop.value;
  }
  console.log("getScrollHistory: ", scrollTop.value);
});
onBeforeRouteLeave((to, from, next) => {
  console.log("scroll: ", document.querySelector(".van-list").scrollTop);
  console.log("body srollTop: ", document.body.scrollTop);
  if (courseList.value.length) {
    scrollTop.value = document.querySelector(".van-list").scrollTop;
  } else {
    scrollTop.value = 0;
  }
  console.log("scrollTopHistory: ", scrollTop.value);
  next();
});
</script>
