...
const getWaterBallOpts = (numerator: number, denominator: number) => {
    ...
    var rate = denominator === 0 ? 0 : Number((numerator / denominator).toFixed(1))
    const opts = {
        title: [{
            bottom: '0%',
            left: 'center',
        ...
        },
        {
            ...
            data: [
                {
                    name: '',
                    value: denominator == 0 ? 0 : ((denominator - numerator) / denominator) * 100, // 已经完成的学分
                    //value: denominator == 0 ? 0 : (1 - (numerator / denominator)) * 100,
                    labelLine: {
                        show: false,
                    },
                    itemStyle: {
                ...
                },
                {
                    //画剩余的刻度圆环
                    name: '',
                    value: denominator == 0 ? 100 : (numerator / denominator) * 100,
                    itemStyle: {
                        color: '#ecebea',
                    },
                    label: {
...
const initwaterBallChart = () => {
    if (!waterBallChart) {
        waterBallChart = init(chart.value)
