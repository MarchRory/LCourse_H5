...
<template>
    <div class="container">
            ...
            <div class="myInput">
                <van-cell-group inset>
                    <van-field v-model="keyWords" center clearable placeholder="请输入关键词搜索课程">
                        <template #button>
                            <van-icon name="search" color="#3C3A36" @click="getList(keyWords)" size="22" />
                        </template>
                    </van-field>
                </van-cell-group>
            </div>
            <div class="tag">
                <span style="font-weight: 300;line-height: 21px;font-size: 16px; width: 37px;">分类: </span>
                <div>
                    <van-tag mark color="#65AAEA" size="medium" v-for="(item, index) in tags" :key="index"
                        @click="search(item.key)">
                        {{ item.tag }}
                    </van-tag>
                </div>
            </div>
        ...
        <div class="list">
            <van-list v-model:loading="loading" v-model:error="error" error-text="请求失败，点击重新加载" @load="getList">
                <van-cell v-for="(item, index) in lessons" :key="index" :title="item">
                </van-cell>
            </van-list>
        </div>
    </div>
