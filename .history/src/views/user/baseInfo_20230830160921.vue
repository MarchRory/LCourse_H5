      ...
      <van-form @submit="onSubmit">
        <van-cell-group inset>
          ...
          <van-field v-model="myDepartmentName" readonly name="picker" label="学院" placeholder="选择学院"
            @click="handleShowDepartment" :rules="[{ required: true, message: '请选择学院' }]" />
        </van-cell-group>

        <van-cell-group inset title="联系方式（任意）">
          <van-field v-model="contact['电话']" label="手机号" placeholder="输入手机号" />
          <van-field v-model="contact['QQ']" label="QQ" placeholder="输入QQ号" />
          <van-field v-model="contact['微信']" label="微信" placeholder="输入微信号" />
          <van-field v-model="contact['邮箱']" label="邮箱" placeholder="输入邮箱" />
        </van-cell-group>
...
const contact = JSON.parse(JSON.stringify(userStore.contact))
const year = ref('')
  ...
  userApi
    ...
    .then((res) => {
      if (res.code == 200) {
        ...
        userStore.enrollmentYear = enrollmentYear.value.substring(0, 4)
