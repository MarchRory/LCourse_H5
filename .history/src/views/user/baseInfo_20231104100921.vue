...
const loadMajorColumns = () => {
  const departmentId = departmentColumns.value.find((department) => {
    console.log("my: ", myDepartmentName.value, " compare: ", department.value);
    return department.value.text == myDepartmentName.value;
  });
  /*   getMajorList({ page: 1, pageSize: 20, departmentId:departmentId })
    .then(({ data }) => {
