...
const loadMajorColumns = () => {
  const departmentId = departmentColumns.value.find((department) => {
    console.log(
      "department: ",
      department,
      " isSuccess: ",
      department.text == myDepartmentName.value
    );
    return department.text == myDepartmentName.value;
  });
  /*   getMajorList({ page: 1, pageSize: 20, departmentId: departmentId }).then(
    ({ data }) => {
...
watch(
  ...
  (newVal) => {
    console.log(newVal, majorColumns.value);
