  ...
  <div>
    ...
    <div class="details" v-if="!skeLoad">
      <div class="details-top">
        ...
        <div v-show="detailsObj.signUpstate == 2" class="tools" @click="showCenter = true">
      ...
      <div class="mainInfo">
        ...
        <div class="application">
          ...
          <div v-if="detailsObj.signUpstate == 3" @click="toComment" style="display: flex; flex-direction: column; align-items: center; color: rgba(252, 131, 26, 0.879);
                margin-top: 10px;">
        ...
        <van-popup v-model:show="showCenter" round :style="{ padding: 'px', overflow: 'hidden' }"
          :transition-appear="true" :close-on-click-overlay="false">
          <div class="signBox">
            <div class="signTitle">
              签到码签到
            </div>
...
const checkStaus = () => {
  // 按钮状态
  if (detailsObj.value.numberLimit == detailsObj.value.signUpCount && detailsObj.value.signUpstate != 2 && detailsObj.value.state == 2) {
    btnState.value = 0
    btnContent.value = '课程已满员'
  } else if (detailsObj.value.signUpstate === 0) {
    btnState.value = detailsObj.value.state !== 2 ? 0 : 1
    btnContent.value = detailsObj.value.state !== 2 ? '当前无法报名' : '报名课程'
  } else if (detailsObj.value.signUpstate === -1) {       // 报名被打回, 未通过审核
    btnState.value = 0
    btnContent.value = '未通过审核'
  } else if (detailsObj.value.signUpstate === 1) {        // 报名成功, 等待审核
    btnState.value = 0
    btnContent.value = '等待录取'
  } else if (detailsObj.value.signUpstate === 2) {        // 通过报名
    btnState.value = 0
    btnContent.value = detailsObj.value.state !== 3 ? '等待课程开始' : '等待签到'
