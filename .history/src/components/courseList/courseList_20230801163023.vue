      ...
      rqCourse
        ...
        .then((res: any) => {
          if (res.code == 200) {
            ...
            this.skeleton = false;
            console.log(Boolean(data.total))
            this.$emit('findState', Boolean(data.total))
...
<template>
  <div>
    <div class="list">
      <van-list v-if="propList && propList.length">
        <van-cell v-for="(course, index) in propList" :key="index" @click="openDetail(course.id)">
          <course-preview :course="course"></course-preview>
        </van-cell>
      </van-list>

      <van-list v-else v-model:loading="loadStatus.loading" :finished="loadStatus.finished"
        v-model:error="loadStatus.error" error-text="请求失败，点击重新加载" finished-text="没有更多了" @load="loadList">
