...
const loadSelfCommentTemplateList = () => {
  getCommentTemplateAPI(commentTemplateListParams.value).then(
    (res: Response<commentTemplateResultModel>) => {
      ...
      console.log(selfCommentTemplateList.value);
    }
  );
};
watch(
...
<template>
  <div>
    ...
    >
      <div>
        <div v-for="(template, index) in selfCommentTemplateList" :key="index">
          <article class="templateItem">
            <div v-for="(item, index) in template.templateText" :key="index">
              <span
                v-if="(item as string).includes('${')"
                class="selectVocabularyArea"
              ></span>
              <span style="display: inline; width: fit-content" v-else>
                {{ item }}</span
              >
            </div>
          </article>
