...
const emits = defineEmits<{
  (e: "onChange", chosenTextObj: commentTemplate): void;
}>();
...
const insertedDOM = ref<HTMLDOM<HTMLInputElement>>();
...
function insertText(
  ...
  if (!isOpenTemplateList.value) {
    insertedDOM.value = e.target as unknown as HTMLDOM<HTMLInputElement>;
    vocabularyParams.value = params;
    isOpenVocabularyList.value = true;
  }
}
function insertWord(wordObj: vocabularyType) {
  insertedDOM.value!.target.value = wordObj.word;
}
...
function chooseTemplate(templateObj: commentTemplate) {
  templateTextCopy.value = templateObj.templateText;
