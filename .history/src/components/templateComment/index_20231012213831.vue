...
const loadSelfCommentTemplateList = () => {
  ...
  getCommentTemplateAPI(commentTemplateListParams.value)
    .then((res: Response<commentTemplateResultModel>) => {
      ...
      list.forEach(async (item: commentTemplate) => {
        item.templateText = await (item.templateText as string).split("&");
...
const templateListFinished = computed(() => {
  return Boolean(
    selfCommentTemplateList.value.length &&
