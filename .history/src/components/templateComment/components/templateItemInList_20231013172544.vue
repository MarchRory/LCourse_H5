...
const templateItem = ref<Element>();
const completeHeight = ref();
watch(
  () => isReadMore.value,
  () => {
    completeHeight.value = templateItem.value?.clientHeight;
    console.log(window.getComputedStyle(templateItem.value as Element));
    // templateItem.value?.style.height = completeHeight.value
    console.log(completeHeight.value);
  }
);
</script>
