...
import { courseStateChangeType, TypeMap } from './utils/msgType/course/courseMsg'
...
const courseStatusChange = TypeMap.courseStateChange          // 课程状态改变的type, 和courseTypeMap中那个一样, 
const wsStore = ref()
export class WebSocketEventBucket implements webSocketBucketType {
    ...
    useMethod(type: number | string | TypeMap, e: wsDataType | mySocket) {
        if (courseStateChangeType.includes(type as TypeMap)) {
