...
class mySocket extends WebSocket {
    ...
    constructor(bucket: webSocketBucketType | null, token: string, receivedURL?: string) {
        let wsURL = receivedURL ? receivedURL : DEFAULT_WS_URL
        super(wsURL)
        super.addEventListener('beforeSend', (e) => {
            e.target.setRequestHeader('swpu_token', token)
        })
        this.$bucket = bucket
        this.initSocket()
    }
    ...
    initSocket() {
        ...
        super.onclose = (e) => {
            if ($retryCnt < DEFAULT_RETRIES_CNT) {
                ...
                const socket = new mySocket(this.$bucket, token)
