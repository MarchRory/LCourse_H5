  ...
  <div class="results">
    ...
    <div v-if="isFound" class="results-content">
      ...
      <ul class="results-list">
        <li
          @click="toDetailsBnt(v.id, v.isSignUp)"
          class="list-item"
          v-for="(v, i) in resultsArr"
          :key="i"
        >
          <div class="item-top">
            <img
              class="item-img"
              src="../../assets/imgs/Cool-Kids-Discussion.png"
              alt=""
            />
          ...
          <div class="item-bottom">
            <p class="time">
              <span>{{ v.hostingStart }}</span
              >到<span>{{ v.hostingEnd }}</span>
            </p>
            <h3>{{ v.title }}</h3>
...
//跳转详情
const toDetailsBnt = (courseId: number, isSignUp: boolean) => {
  router.push({
    path: "/detail",
    ...
    rq.getCourses({
      ...
      if (res.code === 200) {
        if (res.data.list.length < 1) {
          ...
          resultsArr.value = res.data.list;
          isFound.value = true;
          searchVal.value = "";
        }
      }
