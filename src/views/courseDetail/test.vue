...
import { CourseCardQuery } from '@/api/types/courses';
...
const coreInfoConfig = computed(() => {
    ...
    let coreInfo = [
        ...
        { label: '修读权限', content: `${canJoin ? '可修读' : '不可修读'}`, icon: `${canJoin.value ? 'tabler:zoom-check-filled': 'tabler:zoom-candle-filled'}`, iconColor: `${canJoin ? "#0fd041" : "#6b6b6b"}` },
...
const submit = () => {
    ...
    if (courseQuery.value.disabled || ![CourseStateEnum.ing, CourseStateEnum.finished].includes(state)) {
        return;
    }

    if (CourseSignUpStateEnum.completeSign === signUpstate) {
        toComment()
    }else if(CourseSignUpStateEnum.admitted === signUpstate) {
        joinIn()
    }
}
...
<template>
    <div class="pageContainer">
        <XdHeader title="课程详情" back-path="/home" />
